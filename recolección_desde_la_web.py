...
def scraper_poblacion():
    ...
    tabla = soup.find('div', {'class': 'establishment'})
    ...
    for fila in tabla.find_all('tr')[1:]:  # [1:] para saltar el encabezado
        ...
        if len(columnas) > 2:
            ...
            datos = {
                "Ranking": columnas[0].get_text(strip=True),
                "País": columnas[1].get_text(strip=True),
                "Población": columnas[2].get_text(strip=True).replace('\xa0', ''), # Limpia espacios especiales
                "Porcentaje": columnas[3].get_text(strip=True)
            }
            filas.append(datos)
